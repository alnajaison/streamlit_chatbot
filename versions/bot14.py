...
voice_input = mic_recorder(start_prompt="🎙️ Speak now", stop_prompt="⏹️ Stop", just_once=True, key="voice")

if voice_input and "bytes" in voice_input:
    audio_bytes = voice_input["bytes"]

    # Step 1: Convert WebM to WAV using pydub
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as wav_file:
            ...
            # Step 2: Use speech_recognition to transcribe
...
# Choose voice input if available, else use text
if voice_input and voice_input.get("text"):
    prompt = voice_input["text"]
elif text_input:
    prompt = text_input
else:
    prompt = None
